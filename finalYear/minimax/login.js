...
function login(){
  ...
  .then((userCredential) => {
    ...
    db.ref('game/' + userEmail + '/game1').set('?')
    db.ref('game/' + userEmail + '/game1').on('value', snap => questionMark.innerText = snap.val());
    db.ref('game/' + userEmail + '/game2').set('?');
    db.ref('game/' + userEmail + '/game2').on('value', snap => questionMark.innerText = snap.val());
    db.ref('game/' + userEmail + '/game3').set('?');
    db.ref('game/' + userEmail + '/game3').on('value', snap => questionMark.innerText = snap.val());
